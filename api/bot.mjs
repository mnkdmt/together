// Together — Telegram bot webhook on Vercel. Receives Anya's links → ideas and text → shopping.
// (Still single-couple for now: inserts go to COUPLE_ID. Bot multi-tenancy is a follow-up.)
...
bot.on('message:text', async (ctx) => {
  ...
  if (!url) {
    ...
    const { data, error } = await supa.from('shopping')
      .insert(items.map((t) => ({ couple_id: COUPLE_ID, text: t, theme: 'other', author }))).select('id');
